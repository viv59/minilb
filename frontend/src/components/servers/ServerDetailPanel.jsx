    ...
    return (
        <Card>
                ...
                {TABS.map((t) => (
                    <button
                        ...
                        className={`relative pb-2.5 text-xs ${
                            tab === t
                                ? "text-app-text after:absolute after:inset-x-0 after:-bottom-px after:h-0.5 after:rounded after:bg-gradient-to-r after:from-accent1 after:to-accent2"
                                : "text-text-faint"
                        }`}
                    >
                        {t}
            ...
            {tab === "Overview" ? (
                ...
                <div className="py-5 text-xs text-text-faint">
                    {tab} view — plug in your own data here.
                </div>
            )}
...
function Row({ k, v, bar, valueClass = "", last = false }) {
    return (
        <div
            className={`flex items-center justify-between py-2 ${last ? "" : "border-b border-app-text/5"}`}
        >
            <span className="text-text-faint">{k}</span>
            <span className={`font-mono text-xs ${valueClass}`}>
                {bar != null && (
                    <span className="mr-2 inline-block h-1.5 w-20 overflow-hidden rounded-full bg-app-text/10 align-middle">
                        <span
                            className="block h-full bg-gradient-to-r from-accent1 to-accent2"
                            style={{ width: `${bar}%` }}
                        />
                    </span>
                )}
